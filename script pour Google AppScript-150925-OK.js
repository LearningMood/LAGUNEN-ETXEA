...
const SHEET_NAMES = ['site_meta','sections_meta', 'tarif', 'tarif_textes','remarques', 'equipements', 'photos', 'avis', 'transport', 'faq', 'contact'];
...
function readFAQ(sheet){
  ...
  if (!rows.length) {
    return [{ id:'faq', section:'faq', titre:'', description:'', questions:[] }];
  }
  const headers = rows.shift().map(h => String(h||'').trim().toLowerCase());

  const ciTitre = headers.indexOf('titre');
  const ciDesc  = headers.indexOf('description');
  const ciQ     = headers.indexOf('questions');
  const ciA     = headers.indexOf('reponse');

  let titre = '', description = '';
  if (ciTitre >= 0 && rows[0]) titre = rows[0][ciTitre] || '';
  if (ciDesc  >= 0 && rows[0]) description = rows[0][ciDesc] || '';
  // fallback historique C2/D2 si pas de colonnes
  if (!titre)       titre = sheet.getRange(2,3).getValue() || '';
  if (!description) description = sheet.getRange(2,4).getValue() || '';

  const questions = [];
  rows.forEach(r => {
    if (rowIsAllBlank(r)) return;
    if (ciQ >= 0 && ciA >= 0) {
      const q = r[ciQ], a = r[ciA];
      if (!isBlank(q) || !isBlank(a)) {
        questions.push({ question: String(q||'').trim(), reponse: String(a||'').trim() });
      }
    }
  });
...
function doGet(e){
  ...
  // clé/valeur
  if (name === 'site_meta' || name === 'sections_meta' || name === 'tarif_textes') {
